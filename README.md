...
1. https://openapi.nexon.com 접속 후 넥슨 아이디로 로그인
2. 마이페이지 → Nexon Open API → 애플리케이션 등록
3. 게임: 메이플스토리 / 개발 환경: WEB / URL은 아직 없으면 대충 아무 URL이나 입력해도 돼요
4. 등록 후 애플리케이션 상세 페이지에서 API 키 확인
...
1. 이 폴더를 GitHub 저장소로 올리기 (private 저장소로 올려도 됨 — API 키는 코드에 없고 환경변수로만 넣으니 안전해요)
...
5. 배포 완료되면 `https://xxxx.onrender.com` 같은 주소가 생겨요 — 이 주소를 메소로그 사이트의 "서버 주소" 설정칸에 입력하면 끝
